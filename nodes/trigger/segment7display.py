...
class Segment7Display(Node):
    ...
    def build(self):
        ...
        self.add_field("COLOR", ColorField(
            "COLOR",
            parent=self.alias
        ))
...
class ColorField(Field):
    ...
    def build(self, user_data: dict[str, Any] = None):
        ...
        self.dpg_plot = dpg.add_plot(parent=self.dpg_group,
                                     no_menus=False,
                                     no_title=True,
                                     no_box_select=True,
                                     no_mouse_pos=True,
        ...
        default_x = dpg.add_plot_axis(axis=0, no_gridlines=False, no_tick_marks=True, no_tick_labels=True,
                                      lock_min=True, parent=self.dpg_plot)
        ...
        self.__segments.append(DisplaySegment(pmin=[2.5, 8.5], pmax=[4.5, 8.5],  # SEGMENT_A
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[5.5, 7.5], pmax=[5.5, 5.5],  # SEGMENT_B
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[5.5, 3.5], pmax=[5.5, 1.5],  # SEGMENT_C
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[2.5, 0.5], pmax=[4.5, 0.5],  # SEGMENT_D
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[1.5, 3.5], pmax=[1.5, 1.5],  # SEGMENT_E
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[1.5, 7.5], pmax=[1.5, 5.5],  # SEGMENT_F
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[2.5, 4.5], pmax=[4.5, 4.5],  # SEGMENT_G
                                              parent=self.dpg_plot))

        self.__segments.append(DisplaySegment(pmin=[6.5, 0.5], pmax=[6.5, 0.5],  # SEGMENT_DP
                                              parent=self.dpg_plot))
...
class DisplaySegment:
    ...
    def on(self):
        ...
        self.dpg_rect = dpg.draw_rectangle(pmin=self.pmin, pmax=self.pmax, color=self.COLOR_RED, parent=self.parent)

    def off(self):
        dpg.delete_item(self.dpg_rect)
        self.dpg_rect = dpg.draw_rectangle(pmin=self.pmin, pmax=self.pmax, color=self.COLOR_BLACK, parent=self.parent)
